...
class ConfigServers extends EventEmitter {
  ...
  start() {
    ...
    return new Promise(function(resolve, reject) {
      co(function*() {
        ...
        yield Promise.all(self.managers.map(server => server.start()));
  ...
  arbiters() {
    ...
    return Promise.all(self.managers.map(server => server.ismaster())).then(results => {
      return results.reduce((arbiters, ismaster, idx) => {
        if (ismaster.arbiterOnly) arbiters.push(self.managers[idx]);
        return arbiters;
      }, []);
    });
  }
  ...
  secondaries() {
    var self = this;
    return Promise.all(self.managers.map(server => server.ismaster())).then(results => {
      return results.reduce((secondaries, ismaster, idx) => {
        if (ismaster.secondary) secondaries.push(self.managers[idx]);
        return secondaries;
      }, []);
    });
  }
  ...
  stop() {
    ...
    return Promise.all(self.managers.map(server => server.stop()));
  }
  ...
  purge() {
    ...
    return Promise.all(self.managers.map(server => server.purge()));
  }
}
